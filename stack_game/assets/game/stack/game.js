assets.minigame.stack = (function () {
  ...
  */
  ...
  function create(kind, className, innerHTML) {
    var d = document.createElement(kind);
    className && (d.className = className);
    innerHTML && (d.innerHTML = innerHTML);
    return d;
  }
  ...
  function GameWorld() {
    ...
    this.canAt = 0;
    this.at = 0;
    this.scorebox = create("div", "scorebox");
    ...
    this.ascore = 0;
    this.gameOver = false;
    append(superObj, [this.canvas,
                         append(this.scorebox, [append(create("div", 0, "Packages: "), this.epscore),
                                 append(create("div", 0, "Lost: "), this.enscore),
                                 append(create("div", 0), [create("span", 0, "Score: "), this.eascore, create("span", 0, " Goal: "), this.goal]),
                                 append(create("div", 0, "Time: "), this.time)
                                ])]);
  ...
  GameWorld.prototype.draw = function () {
    ...
    for (var i = 0, b = this.boxes, len = b.length; i < len; ++i) {
      b[i].draw(ctx);
    }
  ...
  GameWorld.prototype.move = function (e) {
    if (this.gameOver)
      return;
  ...
  GameWorld.prototype.cleanup = function () {
    remove(document.body, [style]);
    remove(superObj, [this.canvas, this.scorebox]);
    this.scorebox = null;
    this.cavnas = null;
    this.ctx = null;
  ...
  GameWorld.prototype.update = function () {
    this.highest = this.height;
    var time = 1.0 / 60,
      now = Date.now();
    if (this.last)
      time = (now - this.last) / 1000;
    this.last = now;
    this.latime -= time;
    if (this.latime < 0) {
      this.time.innerHTML = Math.round((this.atime -= -this.latime + 0.1) * 100) / 100;
      this.latime = 0.1;
      if (this.gameOver) {
        this.cleanup();
    ...
    if (!this.can) {
      this.can = this.canAt < now;
      if (this.can) {
        this.fake.i = ((Math.random() * numBoxes) | 0);
        //console.log(loadList[this.fake.i].w);
        this.fake.w = (loadList[this.fake.i].w >> 1) << 1;
        this.fake.h = (loadList[this.fake.i].h >> 1) << 1;
      }
    }
    world.Step(time, 1);
    for (var i = 0, b = this.boxes, len = b.length; i < len; ++i) {
  ...
  function Box(world, x, y, width, height, fixed, img) {
    this.box = createBox(world, x, y, width, height, fixed);
    this.width = width * 2;
  ...
  Box.prototype.update = function () {
    var pt = this.box.GetCenterPosition();
    if (pt.y > game.height) {
      world.DestroyBody(this.box);
      game.eascore.innerHTML = (game.ascore -= 2);
      game.enscore.innerHTML = ++game.nscore;

      //    this.box.Destroy();
      splice(game.boxes, this);
      return false;
    } else if (pt.y < game.highest) {
      game.highest = pt.y;
    }
    return true;
  };
  ...
  var toLoad = [["platform", "./stack_game/assets/game/stack/image/platform.png", false], ["box1", "./stack_game/assets/game/stack/image/box1.png", true], ["box2", "./stack_game/assets/game/stack/image/box2.png", true], ["box3", "./stack_game/assets/game/stack/image/box3.png", true], ["box4", "./stack_game/assets/game/stack/image/box4.png", true], ["box5", "./stack_game/assets/game/stack/image/box5.png", true]],
    loaded = {},
    loadNum = 0,
    loadList = null,
    numBoxes = 0;
  ...
  function StackingGame(obj, getGame) {
    ...
    loadNum = toLoad.length;
    loadList = new Array(loadNum);
    numBoxes = 0;
    loaded = {};
    ...
    game = new GameWorld();
    game.promise = new Promise(function (resolve, reject) {
  ...
  GameWorld.prototype.callback = function (didwin) {
  ...
  function loadComplete() {
    ...
    //game.boxes.push(new Box(world, 150, 50, 24, 24, false));
    game.boxes.push(new Box(world, game.width / 2, game.height - 12, 100, 12, true, loaded.platform.img));
    game.update();
  }
